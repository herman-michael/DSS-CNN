...
DEFAULT_IMAGE_FOLDER_PATH = Path('dataset/multiaperture/')
#DEFAULT_IMAGE_FOLDER_PATH = Path('dataset/multiaperture_noaug/')
...
class RegressionTaskData:
    ...
    def make_trainloader(self, batch_size: int = 64) -> torch.utils.data.DataLoader:
        ...
        """
        image_targets, image_classes = load_image_targets_from_csv(self.image_folder_path / 'ma_train.csv')
    ...
    def make_testloader(self, batch_size: int = 64) -> torch.utils.data.DataLoader:
        ...
        """
        image_targets, image_classes = load_image_targets_from_csv(self.image_folder_path / 'ma_test.csv')
        test_data = RegressionImageFolder(
            str(self.image_folder_path / 'test'), 
            image_targets, image_classes,
            transform=self.test_transforms
        )
        test_sampler = SubsetRandomSampler(torch.randperm(len(test_data))[:26317])
        # This constructs the dataloader that actually determines how images will be loaded in batches
    ...
    def make_validloader(self, batch_size: int = 64) -> torch.utils.data.DataLoader:
        ...
        """
        image_targets, image_classes = load_image_targets_from_csv(self.image_folder_path / 'ma_valid.csv')
    ...
    def visualise_image(self):
        ...
        """
        images, targets, classes = next(iter(self.trainloader))
