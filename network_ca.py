...
from data_loading_ca import RegressionTaskData
...
def train_network(model, device, n_epochs: int = 10,
    ...
    optimizer = torch.optim.Adamax(model.parameters(), lr=2e-3, weight_decay=0)
    ...
    scheduler = CosineAnnealingWarmRestartsDecay(optimizer, T_0=4, T_mult=1, decay=0.3)
...
def evaluate_network(model, device,
    ...
    with torch.no_grad():
        ...
        #np.savetxt('ca_noaug_errorimage_alpha.csv', error_image_alpha, delimiter=',')
        #np.savetxt('ca_noaug_errorimage_beta.csv', error_image_beta, delimiter=',')
        
        # Load the error image as a csv file
        #error_image_alpha = np.loadtxt('ca_noaug_errorimage_alpha.csv', delimiter=',')
        #error_image_beta = np.loadtxt('ca_noaug_errorimage_beta.csv', delimiter=',')
...
if __name__ == '__main__':
    ...
    model_select = SparseResNet34(SparseBasicBlock, [3, 4, 6, 3], num_classes=4, class_embedding_size=8192)
