...
from data_loading_ma import RegressionTaskData
...
def train_network(model, device, n_epochs: int = 10,
    ...
    optimizer = torch.optim.Adamax(model.parameters(), lr=3e-3, weight_decay=0)
    ...
    scheduler = CosineAnnealingWarmRestartsDecay(optimizer, T_0=4, T_mult=1, decay=0.4)
    
    warmup_period = 25
    warmup_scheduler = warmup.ExponentialWarmup(optimizer, warmup_period)
    iters = len(regression_task.trainloader)
    warmup_epochs = (warmup_period + iters - 1) // iters
    
    # Configure tensorboard layout
    layout = {
        "DSS-CNN: Multi-Aperture": {
        ...
        for i, (inputs, targets, classes) in enumerate(regression_task.trainloader):
            ...
            with warmup_scheduler.dampening():
                if epoch >= warmup_epochs:
                    scheduler.step(epoch - warmup_epochs + i / iters)
...
def evaluate_network(model, device,
    ...
    with torch.no_grad():
        ...
        plt.hist(error_a_vec*180/pi*max_angle, bins=100, color='blue', alpha=0.5, density=True)
        a_a, loc_a, scale_a = skewnorm.fit(error_a_vec*180/pi*max_angle)
        xmin_a, xmax_a = plt.xlim()
        x_a = np.linspace(xmin_a, xmax_a, 1000)
        p_a = skewnorm.pdf(x_a, a_a, loc_a, scale_a)
        plt.plot(x_a, p_a, 'blue', linewidth=1)
        plt.xlabel('Absolute Error $\\alpha$ Angle (deg)')
        plt.ylabel('Frequency')
        plt.show()
        
        # Plot the error histogram for test B
        plt.hist(error_b_vec*180/pi*max_angle, bins=100, color='blue', alpha=0.5, density=True)
        a_b, loc_b, scale_b = skewnorm.fit(error_b_vec*180/pi*max_angle)
        xmin_b, xmax_b = plt.xlim()
        x_b = np.linspace(xmin_b, xmax_b, 1000)
        p_b = skewnorm.pdf(x_b, a_b, loc_b, scale_b)
        plt.plot(x_b, p_b, 'blue', linewidth=1)
        ...
        np.savetxt('ma_noaug_hist_alpha.csv', error_a_vec*180/pi*max_angle, delimiter=',')
        np.savetxt('ma_noaug_hist_beta.csv', error_b_vec*180/pi*max_angle, delimiter=',')
        ...
        np.savetxt('ma_noaug_actvspred_alpha.csv', actvspred_a.T, delimiter=',',
                   header='Output,Target,Error', comments='') 
        np.savetxt('ma_noaug_actvspred_beta.csv', actvspred_b.T, delimiter=',',
                   header='Output,Target,Error', comments='') 
        ...
        #np.savetxt('ma_noaug_errorimage_alpha.csv', error_image_alpha, delimiter=',')
        #np.savetxt('ma_noaug_errorimage_beta.csv', error_image_beta, delimiter=',')
        
        # Load the error image as a csv file
        #error_image_alpha = np.loadtxt('ma_noaug_errorimage_alpha.csv', delimiter=',')
        #error_image_beta = np.loadtxt('ma_noaug_errorimage_beta.csv', delimiter=',')
